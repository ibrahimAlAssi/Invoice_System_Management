								...
								<div class="dropdown-menu">
									<div class="menu-header-content bg-primary text-right">
										<div class="d-flex">
											...
											<span class="badge badge-pill badge-warning mr-auto my-auto float-left"><a href="markAsRead">تعين قراءة الكل</a></span>
										</div>
                                        <p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 "><h6 style="color: yellow" id="notification_count">{{ auth()->user()->unreadNotifications->count() }}</h6></p>									</div>
                                        <div id="unread_notfication">
                                        @foreach(auth()->user()->unreadNotifications as $notification)
										<div class="main-notification-list Notification-scroll">
                                            <a class="d-flex p-3 border-bottom"  href="{{ route('markAsRead',['id'=>$notification->data['id']]) }}">{{ $notification->data['title'] }} {{ $notification->data['user'] }}</a>
                                                <div class="notifyimg bg-pink">
                                                    <i class="la la-file-alt text-white"></i>
                                                </div>
                                                <div class="mr-3">
                                                    <div class="notification-subtext" style="color: #ef4088;">{{ $notification->created_at }}</div>
                                                </div>
                                            </a>
                                        </div>
                                    @endforeach
